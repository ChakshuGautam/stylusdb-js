...
class Raft extends EventEmitter {
  constructor(address, options = {}) {
    ...
    // Our current state.
    raft.state = Number.isInteger(options.state)
      ? options.state
      : Raft.FOLLOWER;
      ...
      switch (packet.type) {
        ...
        case "voted":
          ...
          if (Raft.CANDIDATE !== raft.state) {
            return write(
              await raft.packet("error", "No longer a candidate, ignoring vote")
            );
          }
        ...
        default:
          if (raft.listeners("rpc").length) {
            ...
            write(
              await raft.packet("error", "Unknown message type: " + packet.type)
            );
          }
      }
    });
  ...
  async commitEntries(entries) {
    ...
    entries.forEach(async (entry) => {
      try {
        ...
        console.log("Error committing entries: ", err);
        errs.push(entry);
      }
      this.emit("commit", entry.command);
    });

    require("fs").writeFileSync("./commit-errors.txt", errs.toString() + "\n");
  }
}
